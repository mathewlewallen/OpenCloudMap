...
export function CategorySelect({
  ...
  function routeToCategoryPage(slug: string) {
    if (slug === "") {
      ...
      router.push(`/library?category=${encodeURIComponent(slug.toLowerCase())}`);
    }
  }
      ...
      <div className="mt-3 sm:mt-4">
        <div className="grid grid-cols-1 lg:hidden">
          <select
            ...
            className="col-start-1 row-start-1 w-full appearance-none rounded-md bg-white py-2 pl-3 pr-8 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus:outline-2 focus:-outline-offset-2 focus:outline-slate-600"
          >
        ...
        <div className="hidden lg:block">
          <nav className="-mb-px flex space-x-4">
            ...
            {categories.map((cat) => (
              <Link
                key={cat}
                href={`/library?category=${encodeURIComponent(cat.toLowerCase())}`}
                aria-current={
