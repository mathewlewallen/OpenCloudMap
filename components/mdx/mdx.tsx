...
import { BgGradient } from "@/components/mdx/BgGradient";
...
function createHeading(level: number) {
  return ({ children }: { children: React.ReactNode }) => {
    // Turn any ReactNode into a plain string
    const text = React.Children.toArray(children).reduce<string>(
      (acc, child) =>
...
function FullWidthCallout({
  ...
  const hasLinks = React.Children.toArray(children).some((child) => {
    ...
    // 2) real React elements with children
    //    here we tell TS that once isValidElement passes,
    //    child is React.ReactElement<{ children?: React.ReactNode }>
    if (
      React.isValidElement<{ children?: React.ReactNode }>(child) &&
      child.props.children
    ) {
      // now TypeScript knows child.props.children is a ReactNode
...
function IdeaQuote({ children }: { children: React.ReactNode }) {
  return <FullWidthCallout type="idea">{children}</FullWidthCallout>;
}

function WarningQuote({ children }: { children: React.ReactNode }) {
  return <FullWidthCallout type="warning">{children}</FullWidthCallout>;
}

function InfoQuote({ children }: { children: React.ReactNode }) {
  return <FullWidthCallout type="info">{children}</FullWidthCallout>;
}

function ThoughtQuote({ children }: { children: React.ReactNode }) {
...
function MDXContent({ code, components }: MDXProps) {
  return (
    <MDXProvider components={{ ...sharedComponents, ...(components || {}) }}>
      <run
        // this prop name is `source` if you’re on v2
        // check your @mdx-js/runtime version
        source={code}
      />
    </MDXProvider>
  );
