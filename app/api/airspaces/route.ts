...
const GEOJSON_URL = `${SUPABASE_URL}/storage/v1/object/public/aerodata/airspaceRaw.json`
...
export async function GET() {
  try {
    ...
    console.error('fetch airspaces failed:', err)
    return NextResponse.json(
      { error: 'Failed to fetch airspaces' },
      { status: 502 }
    )
  }
}